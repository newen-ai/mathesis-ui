...
export default function AteneoCreateGroupPage() {
  ...
  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    window.setTimeout(() => {
      ...
      toast.success("Grupo creado (mock). Continuamos con backend en la próxima fase.");
      router.push("/ateneo");
    }, 250);
  };
          ...
          <div className="mt-5 grid gap-4 sm:gap-5">
            ...
            <section className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] px-4 py-3">
              <label className="flex items-center justify-between gap-3">
                <span>
                  <strong className="block text-scale-4 text-[var(--heading-primary)]">Grupo oficial</strong>
                  <span className="mt-1 block text-scale-2 text-[var(--text-secondary)]">
                    Solo personas autorizadas pueden marcar un grupo como oficial.
                  </span>
                </span>
                ...
                <span className="relative inline-flex h-8 w-14 shrink-0 items-center">
                  <input
                    type="checkbox"
                    checked={form.isOfficialGroup}
                    onChange={(event) => onFieldChange("isOfficialGroup", event.target.checked)}
